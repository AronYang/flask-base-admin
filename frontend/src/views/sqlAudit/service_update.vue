      ...
      <div class="card-body">
        ...
        <template v-for="(db,index) in ruleForm.db_list">
          <el-form-item :label="'步骤'+index" :prop="'db_list.'+index+'.id[1]'" :rules="{
            ...
            <el-button :plain="true" type="danger" @click="db_list_del(index)">删除</el-button>
          </el-form-item>
        </template>
        <el-form-item>
...
export default {
  ...
  methods: {
    ...
    update_flow() {
      ...
      this.$http.put(flowListApiUrl+'/'+id, self.ruleForm).then(function(response) {
    ...
    get_flow(){
      var self = this
      var  id = this.$route.params.id
      this.$http.get(flowListApiUrl+'/'+id).then(function(response){
