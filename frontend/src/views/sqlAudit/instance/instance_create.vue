...
export default {
  ...
  methods: {
    ...
    create_instance() {
      // 提交配置
      var self = this
      this.$http.post(InstanceListApiUrl, this.ruleForm).then(function(response) {
    ...
    get_instance(){
      var copy_id = this.$route.query.copy
      console.log('copy_id',copy_id)
      if (copy_id > 0){
        var self = this
        this.$http.get(InstanceListApiUrl+'/'+copy_id).then(function(response){
