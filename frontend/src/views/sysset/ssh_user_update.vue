...
export default {
  ...
  methods: {
    get_ssh_user_info(){
        ...
        this.$http.get(SyssetSshUserApi+'/'+id).then((res)=>{
          self.ruleForm = res.data.result
        })
    },
    submitForm(formName) {
      // 提交按钮
    ...
    update_ssh_user() {
      var self = this
      var id = self.$route.params.id
      this.$http.put(SyssetSshUserApi+'/'+id, this.ruleForm).then(function(response) {
